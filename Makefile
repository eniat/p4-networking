...
controller: controller.py controller.json p4src/build/nat.json p4src/build/pat.json
...
p4-build: p4src/build/nat.json p4src/build/pat.json

p4src/build/nat.json: p4src/nat.p4
	$(info *** Building nat.p4 program...)
	@mkdir -p p4src/build
	docker run --rm -v ${curr_dir}:/workdir -w /workdir ${P4C_IMG} \
		p4c-bm2-ss --arch v1model -o p4src/build/nat.json \
		--p4runtime-files p4src/build/nat-p4info.txt --Wdisable=unsupported --Wdisable=unused\
		p4src/nat.p4

p4src/build/pat.json: p4src/pat.p4
	$(info *** Building pat.p4 program...)
	docker run --rm -v ${curr_dir}:/workdir -w /workdir ${P4C_IMG} \
		p4c-bm2-ss --arch v1model -o p4src/build/pat.json \
		--p4runtime-files p4src/build/pat-p4info.txt --Wdisable=unsupported --Wdisable=unused\
		p4src/pat.p4
...
docs: util/lib/p4_cli/switch.py util/lib/p4_cli/helper.py
	mkdir -p docs
