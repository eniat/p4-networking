  ...
  node: {nodeId}
    ...
    def start(self, controllers):
        ...
        args = [
            ...
            '-read_req_log_file=%s/read-reqs.txt' % self.tmpDir,
...
class NoOffloadHost(Host):
    ...
    def config(self, **params):
        ...
        for intf in self.intfList():
            if intf.name != 'lo':
                for off in ["rx", "tx", "sg", "gso", "gro", "lro"]:
                    cmd = f"/sbin/ethtool -K {intf.name} {off} off"
                    self.cmd(cmd)
        return r
...
class NoIpv6OffloadHost(NoOffloadHost):
    ...
    def config(self, **params):
        r = super(NoIpv6OffloadHost, self).config(**params)
        for intf in self.intfList():
            if intf.name != 'lo':
                self.cmd(f"sysctl net.ipv6.conf.{intf.name}.disable_ipv6=1")
        return r
